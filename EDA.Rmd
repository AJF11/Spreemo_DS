...
```{r}
exam_data %>%
  count(Exam.ID, Provider.ID) %>% #this part shows the number of instances of each Exam/Provider combination
  ungroup %>%
  count(n) %>% # this part shows how many times we see a given number of Exam/Provider combinations
...
exam_data %>%
  ...
  select(c(1:3,13:16)) %>% # only looking at the columns that do not represent what the reviewer put in
...
This data frame represents the summary feature values for all providers.
...
ggplot(ProviderSummary, aes(x = nExams)) + 
  ...
  theme_light() +
  labs(title = 'Histogram of # of Exams per provider', x = '# of Exams per provider', y = '# of Providers')
```


While 25% of Providers have only 1 exam, a different 25% have 6 or more exams, with one provider having as many as 56.  

Now, I'd like to consider looking at the correlation statistic weighted by # of exams.
...
```{r}
# We first calculate the centroids of the clusters, and unscale/uncenter them to put them on the same range as the original data.
...
### Analyze clusters with other Provider characteristics
...
There seem to be a more significant correlation between `MRI.machine.type` and `cluster2` than there was with the other factors. 90.9% of Stand-Up machines get clustered as bad, while 83.1% of Semi-Open Wide Bore machines get clustered as good.  Open machines seem to be a bit more mixed at 65% good. Closed machines are almost as good as Semi-Open Wide Bore machines, and there are not enough Extremity machines to get a good look.
...
plotData <- predict(logReg1, newdata = plotData, type = 'response') %>%
  ...
  setNames(c('MRI.magnet.strength', 'fitted.y')) # fitting the logistic regression on the dummy data
equipData %>%
  mutate(cluster2 = if_else(cluster2 == 'good', 1, 0)) %>%
  ggplot(aes(x = MRI.magnet.strength, y = cluster2))+
  geom_jitter(height = 0.1, width = 0, alpha = 0.1, aes(size = nExams)) +
# I used the jitter function to show the multiple providers at any given Magnet Strength, and used the opacity to show where they congregated.
