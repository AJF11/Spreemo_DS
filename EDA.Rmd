...
ProviderSummary <- each_exam_data1 %>% 
  ...
  summarize_at(.cols = c('RadPeer.Score', 'Technical.Performance.Score'),
               .funs = c('mean'), na.rm = T)
ProviderSummary <- each_exam_data1 %>%
  count(Provider.ID) %>%
  rename(nExams = n) %>%
...
ProviderSummary <- each_exam_data1 %>% 
  group_by(Provider.ID) %>%
  summarize_at(.cols = vars(Total.Diagnostic.Errors, False.Positive.Count,
                            False.Negative.Count, True.Negative.Count, True.Positive.Count, Neg.Count, 
                            Pos.Count, Tot.Count), sum, na.rm = T) %>%
  inner_join(ProviderSummary, ., by = 'Provider.ID')
ProviderSummary <- each_exam_data1 %>% 
  group_by(Provider.ID) %>%
