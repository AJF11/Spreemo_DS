...
Now we can aggregate the exam date across each provider. I will use mean to average them. 
...
```{r}
paste('Correlation of Means:', round(with(ProviderSummary, cor(x = RadPeer.Score, y = Technical.Performance.Score)), 3))
```
...
```{r}
ProviderSummary %>% 
  ggplot(data = ., mapping = aes(x = RadPeer.Score, y = Technical.Performance.Score, size = nExams)) +
  geom_point() +
  theme_light() +
...
data1 <- ProviderSummary %>%
  ...
  mutate(RadPeer.Score = (RadPeer.Score - toScale$RadPeer.Score_mean) / toScale$RadPeer.Score_sd,
         Technical.Performance.Score = (Technical.Performance.Score - toScale$Technical.Performance.Score_mean) / toScale$Technical.Performance.Score_sd,
         FPR = coalesce((FPR - toScale$FPR_mean) / toScale$FPR_sd, 0),
         FNR = coalesce((FNR - toScale$FNR_mean) / toScale$FNR_sd, 0),
         errR = coalesce((errR - toScale$errR_mean) / toScale$errR_sd, 0),
         w.FPR = coalesce((w.FPR - toScale$w.FPR_mean) / toScale$w.FPR_sd, 0),
         w.FNR = coalesce((w.FNR - toScale$w.FNR_mean) / toScale$w.FNR_sd, 0),
         w.errR = coalesce((w.errR - toScale$w.errR_mean) / toScale$errR_sd, 0))

set.seed(892748923)
kMeans2 <- data1 %>% select(-(1:2)) %>% kmeans(2, nstart = 20)
data1[['cluster1']] <- kMeans2$cluster
kMeans2
```

When creating this cluster, we can see that the clustering accounts for 28.4% of the sum of squares.

We can also weight each point by the number of exams by expanding each Provider to the size of it's number of exams.

```{r}
data2 <- data1 %>% expandRows('nExams')
kMeans2a <- data2 %>%
  select(-Provider.ID, -cluster1) %>%
  kmeans(2, nstart = 20)
...
Here, we account for 28.8% of the sum of squares, which is negligably higher.
