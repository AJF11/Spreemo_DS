...
```{r}
head(exam_data, 10) %>% knitr::kable(align = 'c')
```
...
```{r}
ggplot(data1, aes(x = RadPeer.Score, y = Technical.Performance.Score, size = nExams, color = cluster2)) +
  geom_point() +
  theme_light() +
...
SubSpecial %>% count(Is.Subspecialized, cluster1) %>% mutate(pct = percent(n/sum(n))) %>% knitr::kable(align = 'c')
SubSpecial %>% count(Is.Subspecialized, cluster2) %>% mutate(pct = percent(n/sum(n))) %>% knitr::kable(align = 'c')
```
